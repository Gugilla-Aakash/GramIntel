      ...
      <header style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(7,26,20,0.98)", borderBottom: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 1px 0 rgba(0,0,0,0.1)" }}>
        <div className="shell" style={{ height: 64, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 10 }}>
            ...
            <div aria-label={uiText(lang, "LANGUAGE")} style={{ display: "flex", gap: 3, alignItems: "center" }}>{(["en", "hi", "te"] as const).map((code) => <button key={code} type="button" onClick={() => setUiLang(code)} aria-pressed={lang === code} style={{ color: "#fff", background: lang === code ? "rgba(255,255,255,0.2)" : "transparent", border: "1px solid rgba(255,255,255,0.16)", borderRadius: 999, padding: "6px 8px", fontSize: 10, cursor: "pointer" }}>{code === "en" ? "EN" : code === "hi" ? "हि" : "తె"}</button>)}</div>
